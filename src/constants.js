...
export const SELECT_CURRENT_WEEK =
    "SELECT * FROM userweights WHERE username LIKE ? AND userweights.date BETWEEN strftime('%Y%m%d',date('now','-7 days')) AND strftime('%Y%m%d',date('now'))";
export const SELECT_LAST_WEEK =
    "SELECT * FROM userweights WHERE username LIKE ? AND userweights.date BETWEEN strftime('%Y%m%d',date('now','-14 days')) AND strftime('%Y%m%d',date('now','-7 days'))";
